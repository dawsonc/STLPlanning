...
def test():
    ...
    for wall in _walls:
        ...
        half_length = (
            np.sqrt((((np.array(wall[0]) - np.array(wall[1]))) ** 2).sum()) / 2
        )
        A3 = A0
    ...
    avoid_walls = Node(
        "and", deps=[Node("negmu", info={"A": A, "b": b}) for A, b in walls]
    )
    always_avoid_walls = Node(
        "A",
        deps=[
            avoid_walls,
        ],
    ...
    untils = [
        Node("U", deps=[avoid_door, pick_key], info={"int": [0, tmax]})
        for avoid_door, pick_key in zip(avoid_doors, pick_keys)
    ]

    reach_goal = Node("mu", info={"A": goal[0], "b": goal[1]})
    finally_reach_goal = Node(
        "F",
        deps=[
            reach_goal,
        ],
