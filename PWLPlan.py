...
def always(i, a, b, zphis, PWL):
    ...
    for j in range(len(PWL) - 1):
        ...
        t_j_1 = PWL[j + 1][1]
        conjunctions.append(
            Disjunction([noIntersection(t_j, t_j_1, t_i + a, t_i_1 + b), zphis[j]])
        )
...
def eventually(i, a, b, zphis, PWL):
    ...
    for j in range(len(PWL) - 1):
        ...
        t_j_1 = PWL[j + 1][1]
        disjunctions.append(
            Conjunction([hasIntersection(t_j, t_j_1, t_i_1 + a, t_i + b), zphis[j]])
        )
...
def bounded_eventually(i, a, b, zphis, PWL, tmax):
    ...
    return Disjunction(
        [
            Conjunction([z_intervalWidth, Disjunction(disjunctions)]),
            t_i + b - tmax - EPS,
        ]
    )
...
def until(i, a, b, zphi1s, zphi2s, PWL):
    ...
    for j in range(len(PWL) - 1):
        ...
        for l in range(j + 1):
            ...
            t_l_1 = PWL[l + 1][1]
            conjunctions.append(
                Disjunction([noIntersection(t_l, t_l_1, t_i, t_i_1 + b), zphi1s[l]])
            )
...
def release(i, a, b, zphi1s, zphi2s, PWL):
    ...
    for j in range(len(PWL) - 1):
        ...
        for l in range(j):
            ...
            t_l_1 = PWL[l + 1][1]
            disjunctions.append(
                Conjunction([hasIntersection(t_l, t_l_1, t_i_1, t_i_1 + b), zphi1s[l]])
            )
...
def mu(i, PWL, bloat_factor, A, b):
    ...
    for e in range(num_edges):
        ...
        for j in [i, i + 1]:
            ...
            conjunctions.append(
                b[e]
                - np.linalg.norm(a) * bloat_factor
                - sum([a[k] * x[k] for k in range(len(x))])
                - EPS
            )
    return Conjunction(conjunctions)
...
def negmu(i, PWL, bloat_factor, A, b):
    ...
    for e in range(num_edges):
        ...
        for j in [i, i + 1]:
            ...
            conjunctions.append(
                sum([a[k] * x[k] for k in range(len(x))])
                - (b[e] + np.linalg.norm(a) * bloat_factor)
                - EPS
            )
        disjunctions.append(Conjunction(conjunctions))
...
def add_space_constraints(model, xlist, limits, bloat=0.0):
    xlim, ylim = limits
    for x in xlist:
...
def disjoint_segments(model, seg1, seg2, bloat):
    ...
    return (
        0.5 * L1Norm(model, _sub(_add(seg1[0], seg1[1]), _add(seg2[0], seg2[1])))
        - 0.5
        * (
            L1Norm(model, _sub(seg1[0], seg1[1]))
            + L1Norm(model, _sub(seg2[0], seg2[1]))
        )
        - 2 * bloat * np.sqrt(len(seg1[0]))
        - EPS
    )
...
def handleSpecTree(spec, PWL, bloat_factor, size):
    ...
    if spec.op == "mu":
        spec.zs = [
            mu(i, PWL, 0.1, spec.info["A"], spec.info["b"]) for i in range(len(PWL) - 1)
        ]
    elif spec.op == "negmu":
        spec.zs = [
            negmu(i, PWL, bloat_factor + size, spec.info["A"], spec.info["b"])
            for i in range(len(PWL) - 1)
        ]
    elif spec.op == "and":
        spec.zs = [
            Conjunction([dep.zs[i] for dep in spec.deps]) for i in range(len(PWL) - 1)
        ]
    elif spec.op == "or":
        spec.zs = [
            Disjunction([dep.zs[i] for dep in spec.deps]) for i in range(len(PWL) - 1)
        ]
    elif spec.op == "U":
        spec.zs = [
            until(
        ...
        spec.zs = [
            eventually(
                i, spec.info["int"][0], spec.info["int"][1], spec.deps[0].zs, PWL
            )
            for i in range(len(PWL) - 1)
        ]
    elif spec.op == "BF":
        ...
        spec.zs = [
            always(i, spec.info["int"][0], spec.info["int"][1], spec.deps[0].zs, PWL)
            for i in range(len(PWL) - 1)
        ]
    else:
        ...
        if tasks is not None:
            for idx_agent in range(len(tasks)):
                for idx_task in range(len(tasks[idx_agent])):
                    handleSpecTree(
                        tasks[idx_agent][idx_task], PWLs[idx_agent], bloat, size
                    )

            conjunctions = []
            for idx_task in range(len(tasks[0])):
                disjunctions = [
                    tasks[idx_agent][idx_task].zs[0] for idx_agent in range(len(tasks))
                ]
                conjunctions.append(Disjunction(disjunctions))
